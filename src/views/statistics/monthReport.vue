  ...
  <div class="monthReport">
    <div class="app-container">
      ...
      <div
        ...
        style="margin-top: 10px"
      >
        <span class="table-tile">月客流报表</span><span class="table-tile-total">共{{page.total}}条</span>
        <a
...
import { fetchMonthReportData } from '@/api/statistics'
...
export default {
  ...
  data() {
    return {
      ...
      search: {
        dateRange: [new Date(new Date().getTime() - 6 * 3600 * 24 * 30 * 1000), new Date()],
        startDate: formatDate(new Date(new Date().getTime() - 6 * 3600 * 24 * 30 * 1000), 'yyyy-MM-dd'),
  ...
  computed: {
    ...
    exportChart: function () {
      return process.env.BASE_API + `/report/month/export?token=${this.token}&startDate=${this.stTime}&endDate=${this.edTime}&structId=${this.search.structId}&store=${encodeURI(this.storeName)}`
    },
    stTime() {
  ...
  methods: {
    ...
    fetchMonthReportData() {
      ...
      fetchMonthReportData(params).then((res) => {
        this.loading = false
        this.allData = res
        this.tableData = this.allData.slice((this.page.currentPage - 1) * this.page.pageSize, this.page.currentPage * this.page.pageSize)
        ...
        const data = {
          date: echartData.map((data, index) => data.time),
          total: echartData.map((data, index) => Number(data.total))
        }
        this.loadMonthReportChart(data)
      })
    },
    loadMonthReportChart(data) {
      this.chart = echarts.init(document.getElementById('monthReportChart'))
