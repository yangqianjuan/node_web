  ...
  <div class="visitUnrecognized">
    <div class="app-container">
      ...
      <div class="table-prefix-bars">
        <span class="table-tile">不可识别列表</span><span class="table-tile-total">共{{page.total}}条</span>
      </div>
      <div style="background-color: #ffffff; padding-bottom: 10px; min-height: calc(100vh - 215px); border-radius: 5px;">
        <el-table
          ref="visitUnrecognizedTable"
          :data="items"
          row-key="id"
          fit
          v-loading="loading"
...
<script>
import { getVisitUnrecognized, getSearchCondition, getCameraOption, getShopOption } from '@/api/statistics'
...
export default {
  ...
  methods: {
    ...
    getVisitUnrecognized: function (val) {
      this.loading = true
      const params = {
      ...
      getVisitUnrecognized(params).then((res) => {
        this.loading = false
        this.items = []
